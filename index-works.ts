...
const expandableVerticalGrid = document.querySelector('#expandableVerticalGrid') as HTMLElement;
...
if (worksContainer) {
  ...
  const worksElements = worksContainer.getElementsByClassName('works-img') as HTMLCollectionOf<HTMLElement>;

  // Loop through each works element
  Array.from(worksElements).forEach((element: HTMLElement) => {
...
function ExpandWorks(element: HTMLElement){
    //GET STRING
...
function enableGrid(grid: HTMLElement){
    grid.style.pointerEvents = 'all';
}

function disenableGrid(grid: HTMLElement){
...
function appendElementToGrid(fileName: string, isVertical: boolean, folderName: string){
    ...
    .then(response => {
        if (response.ok) {
           ...
           if(isVertical){
            ...
            const childElement= document.createElement('img') as HTMLImageElement;
