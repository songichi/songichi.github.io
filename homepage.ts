import { typeObject } from "./typeData.js";
...
for (var i = TypeGroupListToDisplay.length - 1; i > 0; i--) {
  ...
  var temp: typeObject = TypeGroupListToDisplay[i];
...
for(var i = 0; i < TypeGroupListToDisplay.length; i++){
  ...
  var thisTypeHtml =
    '<div class = "HomepageGridGroup scrollHidden" style = "color:  ' + TypeGroupListToDisplay[i].color + '; stop-color: ' + TypeGroupListToDisplay[i].colorS +' ">' +
...
const observer = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    ...
    if(entry.isIntersecting){
      ...
      ReApplyHeaderColors(
        window.getComputedStyle(entry.target).getPropertyValue("stop-color"), 
        window.getComputedStyle(entry.target).getPropertyValue("color"));
    }else{
...
if (sessionStorage.getItem("recentColor") == null) {
  ...
  //initializing
  var selectedGroup: typeObject = TypeGroupList[0];
...
const homepageBtnAbout = document.getElementById('homepageBtnAbout') as HTMLElement;
...
function LoadHomepage() {
  ...
  (document.getElementById("homepageImage") as HTMLImageElement)!.src =
    recentType.image;
...
var rerollTypeList: typeObject[] = [cloneRecent, cloneRecent, cloneRecent];
...
function typeIsDuplicate(type: typeObject) {
